import { chartsAPI } from '../src/functions/chartsAPI';
...
async function main() {
  try {
    ...
    console.log(`   Latest price: $${ethUsdcLine.data[ethUsdcLine.data.length - 1]?.value || 'N/A'}\n`);
    ...
    if (ethUsdcCandle.data.length > 0) {
      const latest = ethUsdcCandle.data[ethUsdcCandle.data.length - 1];
    ...
    const intervals = [
      ...
      { seconds: 900, name: '15 minutes' },
      { seconds: 3600, name: '1 hour' },
      { seconds: 14400, name: '4 hours' },
      { seconds: 86400, name: '1 day' },
      { seconds: 604800, name: '1 week' }
    ];
