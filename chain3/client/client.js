...
class XRPLEscrowClient {
    ...
    this.http.interceptors.response.use(
      ...
      async (error) => {
        ...
        if (
          ...
          console.log(
            `Retrying request (${config.__retryCount}/${this.retries})...`
          );
  ...
  static hashSecret(secret) {
    ...
    return (
      "0x" + crypto.createHash("sha3-256").update(secretBytes).digest("hex")
    );
  }
  ...
  async waitForStatus(
    ...
    while (Date.now() - startTime < timeout) {
      try {
        ...
        console.log(
          `Waiting for escrow ${escrowId} to reach status "${targetStatus}". Current: "${escrow.status}"`
        );
    ...
    throw new Error(
      `Timeout waiting for escrow ${escrowId} to reach status "${targetStatus}"`
    );
  }
  ...
  async completeEscrowWorkflow(escrowParams, fundingTx, secret, callerAddress) {
    ...
    try {
      ...
      workflow.funding = await this.fundEscrow(
      ...
      const escrow = await this.waitForStatus(
        workflow.escrow.escrowId,
        "funded"
      );
...
/**
 * Utility functions for XRPL escrow operations
 */
