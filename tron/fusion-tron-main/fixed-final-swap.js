...
class FixedFinalSwap {
  constructor() {
    ...
    console.log("🎯 FIXED FINAL ATOMIC SWAP");
    console.log("==========================");
    console.log("Final implementation with address format fix");
    console.log("");
  }
  ...
  async executeFixedSwap() {
    try {
      console.log("🚀 Starting fixed atomic swap execution...");

      // Phase 1: Setup and validation
      await this.setupAndValidate();

      // Phase 2: Create escrows (with FIXED address handling)
  ...
  async setupAndValidate() {
    ...
    // Address handling with proper formatting
  ...
  async createEscrowsFixed() {
    ...
    console.log(
      `   Resolver (eth-format for ID): ${this.tronResolverEthFormat}`
    );
    ...
    // Generate TRON escrow ID with FIXED address handling
    console.log("\n🔧 Generating TRON Escrow ID (FIXED):");
    console.log(`   Using eth-format resolver: ${this.tronResolverEthFormat}`);
    ...
    const tronEscrowId = ethers.keccak256(
      ethers.solidityPacked(
        ...
        [
          ...
          "0x9fd14Nj9j7xAB4dbGeiX9h8unkKHxuWwb", // TRX zero in eth format
...
async function main() {
  console.log("🚀 FIXED FINAL ATOMIC SWAP");
  console.log("Final implementation with address format fixes\n");

  const swap = new FixedFinalSwap();
  await swap.executeFixedSwap();
}
