...
class TronExtension {
     ...
     */
    createTronWebInstance(privateKey) {
     ...
     */
    async deployTronEscrowDst(params, privateKey) {
        ...
        try {
            ...
            const TRON_ESCROW_FACTORY_PATCHED_ADDRESS = this.config.TRON_ESCROW_FACTORY_ADDRESS; // LIVE FACTORY ADDRESS - TESTED AND VERIFIED
            ...
            const immutables = {
                ...
                token: addressToUint256(params.dstAsset === "TNUC9Qb1rRpS5CbWLmNMxXBjyFoydXjWFR"
                    ? "0x0000000000000000000000000000000000000000"
                    : params.dstAsset), // Address as uint256 (0x0 for native TRX)
            ...
            const srcCancellationTimestamp = Math.floor(Date.now() / 1000) + params.timelock;
            ...
            const isNativeTRX = params.dstAsset === "TNUC9Qb1rRpS5CbWLmNMxXBjyFoydXjWFR" ||
                params.dstAsset === "0x0000000000000000000000000000000000000000";
            const totalCallValue = isNativeTRX
                ? (parseInt(params.dstAmount) + parseInt(params.safetyDeposit)).toString()
            ...
            const contract = await tronWeb.contract(factoryABI, TRON_ESCROW_FACTORY_PATCHED_ADDRESS);
            ...
            if (txHash) {
                ...
                console.log("⏳ Waiting for Tron transaction confirmation (this may take several minutes)...");
                let attempts = 0;
                ...
                while (attempts < maxAttempts && !transactionConfirmed) {
                    ...
                    console.log(`🔄 Attempt ${attempts}/${maxAttempts} - waiting ${waitTime / 1000}s for confirmation...`);
                    ...
                    try {
                        ...
                        if (txInfo && Object.keys(txInfo).length > 0) {
                            ...
                            if (txInfo.contract_address) {
                                try {
                                    contractAddress = this.tronWeb.address.fromHex(txInfo.contract_address);
                                    console.log(`✅ Contract address from transaction: ${contractAddress}`);
                                }
                                catch (addrError) {
                                    console.log(`Contract address conversion failed: ${addrError.message}`);
                                }
                            }
                            // Parse event logs to find DstEscrowCreated event - using proven logic
                            if (txInfo.log && txInfo.log.length > 0) {
                                console.log(`🔍 Found ${txInfo.log.length} event log(s) - analyzing...`);
                                for (let i = 0; i < txInfo.log.length; i++) {
                                    ...
                                    try {
                                        ...
                                        console.log(`     Log signature (first 8): ${log.topics?.[0]?.substring(0, 8) || "None"}`);
                                        if (log.topics &&
                                            ...
                                            if (log.data && log.data.length >= 192) {
                                                ...
                                                try {
                                                    contractAddress = this.tronWeb.address.fromHex("41" + addressHex);
                                                    console.log(`     ✅ Escrow Address: ${contractAddress}`);
                                                    console.log(`     Hashlock: 0x${hashlockHex}`);
                                                    console.log(`     Taker: 0x${takerHex}`);
                                                }
                                                catch (addrError) {
                                                    console.log(`     Address conversion failed: ${addrError.message}`);
                                                }
                                            }
                                            else {
                                                console.log(`     Data too short: ${log.data?.length || 0} chars`);
                                            }
                                            if (contractAddress) {
                                                this.logger.success("Escrow address extracted from event", {
                                        ...
                                        console.log(`     Event decode failed: ${eventError.message}`);
                                    }
                                }
                            }
                            else {
                            ...
                            if (!contractAddress && txInfo.receipt?.result === "SUCCESS") {
                                this.logger.warn("Transaction succeeded but no DstEscrowCreated event found - escrow may still be created");
                                console.log(`✅ Transaction succeeded! Check details: https://nile.tronscan.org/#/transaction/${txHash}`);
                            }
                        ...
                        console.log(`⚠️ Attempt ${attempts}: Could not fetch transaction info (${infoError.message})`);
                        // Only fail if we're sure it's a revert, not a timing issue
                        if (infoError.message?.includes("CONTRACT_VALIDATE_ERROR") ||
                ...
                if (!transactionConfirmed) {
                    console.log("⏰ Timeout reached, but transaction may still be processing");
                    console.log(`🔗 Check transaction status: https://nile.tronscan.org/#/transaction/${txHash}`);
                    this.logger.warn("Transaction confirmation timeout - check Tronscan for final status", {
                        txHash,
            ...
            if (errorMessage.includes("InsufficientNativeValue")) {
                const match = errorMessage.match(/InsufficientNativeValue\((\d+), (\d+)\)/);
                if (match) {
                    throw new Error(`Insufficient TRX value: required ${match[1]}, provided ${match[2]}`);
                }
            }
                ...
                throw new Error("Invalid timing: destination cancellation time exceeds source cancellation time");
            }
                ...
                throw new Error("CREATE2 address computation mismatch - TVM compatibility issue");
            }
            else if (errorMessage.includes("Bytecode hash mismatch")) {
                throw new Error("Proxy bytecode hash mismatch - TVM compatibility issue");
            }
     ...
     */
    async deployTronEscrowSrc(params, privateKey) {
        ...
        try {
            ...
            const immutables = {
                ...
                token: addressToUint256(params.srcAsset === "TNUC9Qb1rRpS5CbWLmNMxXBjyFoydXjWFR"
                    ? "0x0000000000000000000000000000000000000000"
                    : params.srcAsset), // Address as uint256 (0x0 for native TRX)
            ...
            // For native TRX: callValue = amount + safetyDeposit
            // For TRC20 tokens: callValue = safetyDeposit only
            const isNativeTRX = params.srcAsset === "TNUC9Qb1rRpS5CbWLmNMxXBjyFoydXjWFR" ||
                params.srcAsset === "0x0000000000000000000000000000000000000000";
            const totalCallValue = isNativeTRX
                ? (parseInt(params.srcAmount) + parseInt(params.safetyDeposit)).toString()
                : params.safetyDeposit;
            ...
            if (txHash) {
                // Wait for Tron transaction confirmation with proper retry mechanism
                console.log("⏳ Waiting for Tron SrcEscrow transaction confirmation...");
                let attempts = 0;
                ...
                while (attempts < maxAttempts && !transactionConfirmed) {
                    attempts++;
                    const waitTime = 10000; // 10 seconds per attempt
                    console.log(`🔄 TronSrcEscrow Attempt ${attempts}/${maxAttempts} - waiting ${waitTime / 1000}s...`);
                    await new Promise((resolve) => setTimeout(resolve, waitTime));
                    // Try to get transaction info and parse event logs for escrow address
                    try {
                        const txInfo = await this.tronWeb.trx.getTransactionInfo(txHash);
                        if (txInfo && Object.keys(txInfo).length > 0) {
                            ...
                            if (txInfo.receipt?.result === "FAILED" &&
                                !txInfo.contract_address) {
                                throw new Error(`TronSrcEscrow transaction failed with result: ${txInfo.receipt?.result}`);
                            }
                            // If we have a contract address from the transaction, use it
                            if (txInfo.contract_address) {
                                try {
                                    contractAddress = this.tronWeb.address.fromHex(txInfo.contract_address);
                                    console.log(`✅ TronSrcEscrow contract address from transaction: ${contractAddress}`);
                                }
                                catch (addrError) {
                                    console.log(`TronSrcEscrow contract address conversion failed: ${addrError.message}`);
                                }
                            }
                            // Parse event logs to find SrcEscrowCreated event
                            if (txInfo.log && txInfo.log.length > 0) {
                                console.log(`🔍 Found ${txInfo.log.length} event log(s) for TronSrcEscrow - analyzing...`);
                                for (let i = 0; i < txInfo.log.length; i++) {
                                    const log = txInfo.log[i];
                                    ...
                                    try {
                                        ...
                                        if (log.topics &&
                                            ...
                                            if (log.data) {
                                                ...
                                                this.logger.success("SrcEscrow creation event detected", {
                                                    txHash,
                                                    logIndex: i,
                                        ...
                                        console.log(`     SrcEscrow event decode failed: ${eventError.message}`);
                                    }
                                }
                            }
                            ...
                            console.log(`🔄 TronSrcEscrow attempt ${attempts}: No transaction info yet...`);
                        }
                    }
                    catch (error) {
                        console.log(`🔄 TronSrcEscrow attempt ${attempts} failed: ${error.message}`);
                    }
                }
                if (!transactionConfirmed) {
                    this.logger.warn("TronSrcEscrow transaction confirmation timeout - proceeding with transaction hash", {
                        txHash,
                        tronscanUrl: `https://nile.tronscan.org/#/transaction/${txHash}`,
                    });
                }
            ...
            if (errorMessage.includes("InsufficientNativeValue")) {
                const match = errorMessage.match(/InsufficientNativeValue\((\d+), (\d+)\)/);
                if (match) {
                    throw new Error(`Insufficient TRX value for SrcEscrow: required ${match[1]}, provided ${match[2]}`);
                }
            }
            else if (errorMessage.includes("InvalidAmount")) {
     ...
     */
    async withdrawFromTronEscrow(escrowAddress, secret, exactImmutables, // Use exact immutables from creation
    privateKey) {
                ...
                try {
                    ...
                    if (txInfo && Object.keys(txInfo).length > 0) {
                        ...
                        if (transactionResult === "SUCCESS") {
                            console.log("✅ Tron withdrawal transaction SUCCEEDED on blockchain");
                            break;
                        }
                        else if (transactionResult === "REVERT" ||
                            transactionResult === "FAILED") {
                            throw new Error(`Tron withdrawal transaction FAILED on blockchain: ${transactionResult}`);
                        }
                    }
                    else {
                        console.log(`🔄 Attempt ${attempts}/${maxAttempts}: Transaction still processing...`);
                    }
                }
                catch (infoError) {
                    if (infoError.message?.includes("FAILED") ||
                        infoError.message?.includes("REVERT")) {
                        throw infoError;
                    }
                    console.log(`⚠️ Attempt ${attempts}: Could not fetch transaction info yet`);
                }
            }
            if (!transactionResult || transactionResult !== "SUCCESS") {
                throw new Error(`Tron withdrawal transaction failed or timeout. Result: ${transactionResult || "UNKNOWN"}`);
            }
     ...
     */
    async cancelTronEscrow(escrowAddress, immutables, privateKey) {
     ...
     */
    async getTronEscrowStatus(escrowAddress) {
        try {
     ...
     */
    createPackedTimelocks(timelock) {
        ...
        const packed = (deployedAt << BigInt(224)) | // Deployment timestamp in top 32 bits
        ...
        this.logger.debug("Created packed timelocks with correct TimelocksLib format", {
     ...
     */
    getTronAddressFromPrivateKey(privateKey) {
        const tronWeb = this.createTronWebInstance(privateKey);
     ...
     */
    async waitForConfirmation(txHash, timeoutMs = 60000) {
     ...
     */
    async getTronTransactionDetails(txHash) {
        try {
            const apiKey = this.config.TRON_API_KEY;
            const response = await fetch(`https://nileapi.tronscan.org/api/transaction-info?hash=${txHash}`, {
                headers: {
...
exports.TronExtension = TronExtension;
//# sourceMappingURL=TronExtension.js.map